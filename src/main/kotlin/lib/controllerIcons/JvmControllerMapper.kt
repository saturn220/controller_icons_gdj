...
@RegisterClass
class JvmControllerMapper : RefCounted() {

    fun convertJoypadPath(
        path: String,
        device: Int,
        fallback: JvmControllerSettings.Devices,
        forceControllerIconStyle: JvmControllerSettings.Devices = JvmControllerSettings.Devices.NONE,
    ): String {
        return when (getJoypadType(device, fallback, forceControllerIconStyle)) {
            JvmControllerSettings.Devices.LUNA -> convertJoypadToLuna(path)
            JvmControllerSettings.Devices.PS3 -> convertJoypadToPs3(path)
            JvmControllerSettings.Devices.PS4 -> convertJoypadToPs4(path)
            JvmControllerSettings.Devices.PS5 -> convertJoypadToPs5(path)
            JvmControllerSettings.Devices.STADIA -> convertJoypadToStadia(path)
            JvmControllerSettings.Devices.STEAM -> convertJoypadToSteam(path)
            JvmControllerSettings.Devices.SWITCH -> convertJoypadToSwitch(path)
            JvmControllerSettings.Devices.JOYCON -> convertJoypadToJoycon(path)
            JvmControllerSettings.Devices.XBOX360 -> convertJoypadToXbox360(path)
            JvmControllerSettings.Devices.XBOXONE -> convertJoypadToXboxOne(path)
            JvmControllerSettings.Devices.XBOXSERIES -> convertJoypadToXboxSeries(path)
            JvmControllerSettings.Devices.STEAM_DECK -> convertJoypadToSteamDeck(path)
            JvmControllerSettings.Devices.OUYA -> convertJoypadToOuya(path)
            JvmControllerSettings.Devices.NONE -> ""
        }
    }
    ...
    fun getJoypadType(
        ...
        fallback: JvmControllerSettings.Devices,
        forceControllerIconStyle: JvmControllerSettings.Devices = JvmControllerSettings.Devices.NONE,
    ): JvmControllerSettings.Devices {
        if (forceControllerIconStyle != JvmControllerSettings.Devices.NONE) {
        ...
        if (!available.contains(deviceToUse.toLong())) {
            deviceToUse = JvmControllerIcons.instance?.lastController ?: deviceToUse
        }
        if (!available.contains(deviceToUse.toLong())) {
        ...
        return when {
            controllerName.contains("Luna Controller") -> JvmControllerSettings.Devices.LUNA
            controllerName.contains("PS3 Controller") -> JvmControllerSettings.Devices.PS3
            controllerName.contains("PS4 Controller") || controllerName.contains("DUALSHOCK 4") -> JvmControllerSettings.Devices.PS4
            controllerName.contains("PS5 Controller") || controllerName.contains("DualSense") -> JvmControllerSettings.Devices.PS5
            controllerName.contains("Stadia Controller") -> JvmControllerSettings.Devices.STADIA
            controllerName.contains("Steam Controller") -> JvmControllerSettings.Devices.STEAM
            controllerName.contains("Switch Controller") || controllerName.contains("Switch Pro Controller") -> JvmControllerSettings.Devices.SWITCH
            controllerName.contains("Joy-Con") -> JvmControllerSettings.Devices.JOYCON
            controllerName.contains("Xbox 360 Controller") -> JvmControllerSettings.Devices.XBOX360
            controllerName.contains("Xbox One") || controllerName.contains("X-Box One") || controllerName.contains("Xbox Wireless Controller") -> JvmControllerSettings.Devices.XBOXONE
            controllerName.contains("Xbox Series") -> JvmControllerSettings.Devices.XBOXSERIES
            controllerName.contains("Steam Deck") || controllerName.contains("Steam Virtual Gamepad") -> JvmControllerSettings.Devices.STEAM_DECK
            controllerName.contains("OUYA Controller") -> JvmControllerSettings.Devices.OUYA
            else -> fallback
        }
    }
